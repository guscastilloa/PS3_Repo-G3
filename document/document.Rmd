...
```{r grap00, echo=FALSE, message=FALSE, out.width="70%" , message=FALSE, fig.cap="Distribución del precio de las viviendas (log)", dpi=300}
...
## Imputación de Datos Faltantes y **Data leakage**

Las variables del número de baños, de habitaciónes, de la superficie cubierta (`surface_covered`) y de superficie total (`surface_total`) presentaban muchos valores faltantes. Como se observa en el cuadro a continuación:
```{r, echo=FALSE, results='asis'}
cat(readLines(here("views/tables/code.tex")), sep = "\n")
```

Por esta razón procedimos a hacer 3 tipos de imputaciones: con el promeido, la mediana y $k$ vecinos cercanos (kNN). Sin embargo, al revisar con detenimiento el ejercicio de imputación de datos faltantes (**missing values**), caímos en cuenta de que todas las imputaciones de los valores faltantes que se usaron (promedio, mediana, K-vecinos cercanos) se hicieron sin tener en cuenta que los datos de entrenamiento no debían tener nada de información relacionada con los datos de prueba. Al observar con detenimiento la imputación realizada observamos que tanto la media como la mediana de las variables a las que se les imputaron datos fueron calculadas sobre el conjunto de datos unidos, tanto los de entrentamiento como los de prueba. Lo mismo ocurrió con el algoritmo de kNN. En ese sentido nuestro conjunto de datos de entrenamiento obtuvo información "colada" (**leaked in**) de los datos de prueba, lo que pudo comprometer la calidad de las predicciones.

```{r grap01, echo=FALSE, message=FALSE, out.width="70%" , message=FALSE, fig.cap="Precio en M2 en Chapinero (precio por millones)", dpi=300}
knitr::include_graphics('../views/g2.jpeg')
```
...
# Conclusiones
El presente estudio realizó un ejercicio de predicción del precio de la vivienda para la localidad de Chapinero en Bogotá Utilizando datos de  https://www.properati.com.co. Para construir el modelo, se empleaó información de esta base de datos e información de distancia calculadas a partir de  Open Source Maps (OSM). Se ejecutaron cinco algoritmos de machine learning (Random Forest, Árboles de decisión, regresión lineal, boosting y Elastict Net) y se seleccionó el mejor utilizando como métrica el Error Absoluto Medio (MAE). 

Se encontró que la oferta de bienes inmubles de Chapinero alberga la mayor oferta de universidades en Bogotá, así como dónde se desarrollan mayor actividad comercial. Asimismo, se encontró que el mejor modelo que predice los precios es el Random Forest debido a que permite capturar relaciones no lineales, además de que es más robusto a sobreajuste por la aleatorización de las variables y del conjunto de datos que utiliza para entrenar en la construcción de los árboles. 
...
# Anexo A: Variables utilizadas en el modelo. 
...
\\hline
rooms\\_median & Número de baños de la vivienda \\\\
\\hline
...
También en algunos modelos se ajustaron los hiperparámetros utilizando validación cruzada espacial para corregir la potencial autocorrelación espacial presente en los datos. Los **folds** consistieron en las diferentes localidades de Bogotá, a excepción de Chapinero. 
